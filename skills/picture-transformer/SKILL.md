...
description: 使用已安装的 PictureTransformer 1.0.6 在 Windows 本地转换单张、多张或目录中的图片，并设置输出格式、目录和压缩率。适用于用户要求使用 PictureTransformer 处理本地图片格式转换；不用于图片内容编辑或输出 HEIC/HEIF。
---
...
3. 使用 `Get-Command pictureTransformer` 确认命令可用。若 PATH 中不存在，再检查默认安装位置 `C:\Program Files\PictureTransformer\pictureTransformer.exe`。两处都找不到时，说明需要安装 PictureTransformer 1.0.6 或提供可执行文件路径；不要自行下载安装。
