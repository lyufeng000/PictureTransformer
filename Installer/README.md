# PictureTransformer 安装包

安装包使用 WiX Toolset 5.0.2 构建，包括标准 Windows Installer MSI 和 Burn 引导程序。

## 构建

在项目根目录运行：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\build-setup.ps1
```

最终结果位于 `dist`：

- `PictureTransformer-Setup.exe`：推荐提供给普通用户。
- `PictureTransformer.msi`：用于 Windows Installer 或集中部署。

## 安装行为

- 默认安装到 `%ProgramFiles%\PictureTransformer`。浏览按钮选择父文件夹后，输入框会立即补全 `PictureTransformer` 并显示实际使用的完整安装目录；也可以直接编辑完整路径。
- 图形界面程序位于安装主目录，命令行程序及其依赖集中在 `bin` 子目录。
- 安装前会询问是否将 `bin` 加入系统 PATH，默认勾选；取消勾选则不注册。
- 按计算机安装，需要管理员权限。
- 创建带 PictureTransformer 图标的桌面和开始菜单快捷方式。
- 支持升级、修复和卸载。

## 发布前注意

当前版本号为 `1.0.8`。发布新版本时，需要同步修改 `Directory.Build.props`、`Package.wxs` 和 `Bundle.wxs` 中的版本号。

当前产物没有数字签名，Windows 可能显示“未知发布者”。正式公开分发前建议为 MSI、Burn 引擎和最终 Setup 添加代码签名。
